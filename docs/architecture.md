...
| `PreToolUse` (matcher `Edit\|Write\|Bash\|Shell\|StrReplace`) | The **task gate**: BLOCKS code edits when no task is active. `Shell`/`StrReplace` are the grok tool names — same gate, every provider. |
